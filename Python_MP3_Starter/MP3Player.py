...
class MusicQueue:
    def __init__(self, root, window, music_player):
        ...
        self.initialize_gui()
        self.music_player.queue = self
        self.p_list = []  # Add this line to initialize an empty list


    def initialize_gui(self):
        # Window
        self.Frame_queue = Frame(root, bd=0, relief=RIDGE)
        self.Frame_queue.place(x=485, y=10, width=100, height=250)


        self.List_queue = Listbox(self.Frame_queue, font=("Times new roman", 20),
                                  bg="#333333", fg="grey", selectbackground="lightblue", cursor="hand2",
                                  bd=0)
        #self.playlist_setup.p_list.bind("<<ListboxSelect>>", self.on_playlist_selected)


        # Buttons
        self.createQueueButton = Button(root, text="Add to Queue", width=12, height=2,
                                        font=("times new roman", 10, "bold"),
                                        fg="Black", bg="#333333")
        self.createQueueButton.place(x=375, y=10)


        self.createQueueButton = Button(root, text="Remove from Queue", width=12, height=2,
                                        font=("times new roman", 10, "bold"),
                                        fg="Black", bg="#333333")
        self.createQueueButton.place(x=375, y=50)


        self.createQueueButton = Button(root, text="Clear Queue", width=12, height=2,
                                        font=("times new roman", 10, "bold"),
                                        fg="Black", bg="#333333")
        self.createQueueButton.place(x=375, y=90)


        self.createQueueButton = Button(root, text="Shuffle", width=12, height=2,
                                        font=("times new roman", 10, "bold"),
                                        fg="Black", bg="#333333")
        self.createQueueButton.place(x=375, y=130)


        self.createQueueButton = Button(root, text="Loop", width=12, height=2,
                                        font=("times new roman", 10, "bold"),
                                        fg="Black", bg="#333333")
        self.createQueueButton.place(x=375, y=170)


    def on_playlist_selected(self, event):
        selected_index = self.window.playlist.p_list.curselection()
        if selected_index:
            selected_song_index = int(selected_index[0])
            selected_song = self.window.playlist.p_list.get(selected_song_index)
            # Now you can use selected_song as needed
            self.add_song_to_queue(selected_song)
    ...
    def add_song_to_queue(self, song):
        ...
        # Update the List_queue with the songs in the queue
        self.update_queue_listbox()


    def update_queue_listbox(self):
        # Clear the existing items in the List_queue
        ...
        # Add each song in the queue to the List_queue
        for song in queue:
            self.List_queue.insert(END, song)


    def get_queue(self):
        # Add this method to return the queue in the playlist
        return self.playlist.get_queue()
    
    def get_selected_playlist(self):
        selected_index = self.p_list.curselection()
        if selected_index:
            selected_playlist = self.p_list.get(selected_index)
...
class Window:
    ...
    def initialize_gui(self):
        ...
        def create_equalizer():
            ...
            #dropdown = Toplevel(self.root)
            
            # Create Frame widget inside the Toplevel widget
                #Equalizer menu 
        ...
        # Create a button that opens the dropdown when clicked
        button = Button(self.root, text="Equalizer", command=create_equalizer)
        button.pack()
...
if __name__ == "__main__":
    ...
    music_queue = MusicQueue(root, window, music_player)   
